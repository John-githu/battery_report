...
def _msg(text: str, title: str = "电池深度分析", style: int = 0x40):
    try:
        import ctypes
...
def query_live_battery_status() -> dict:
        ...
        try:
            result = subprocess.run(
                ["wmic", "path", "Win32_Battery", "get",
                 ...
                 "ChargeRate,DischargeRate,Voltage", "/value"],
                capture_output=True, text=True, timeout=8, shell=False,
            )
    ...
    status_map = {
        1: "放电中", 2: "接通 AC 电源", 3: "已充满", 4: "低电量",
        5: "极低电量", 6: "充电中", 7: "高电量充电中", 8: "低电量充电中",
        9: "极低电量充电中", 10: "状态未定义", 11: "部分充电",
    }
...
def infer_power_status_from_report(recent_usage: Sequence[dict]) -> dict:
    ...
    if source == "ac":
        ...
        if previous and latest_pct is not None and latest_pct > previous.get("pct", latest_pct):
            ...
            return {"label": "充电中（根据最近记录推断）", "detail": detail, "charge_percent": f"{latest_pct}%"}
        return {
            "label": "接通 AC 电源",
...
def parse_report(path: str) -> dict:
    ...
    data = {
        ...
        "batteries": parse_installed_batteries(find_table_after_h2(root, "Installed batteries")),
        "recent_usage": parse_recent_usage(find_table_after_h2(root, "Recent usage")),
        "battery_usage": parse_battery_usage(find_table_after_h2(root, "Battery usage")),
        "usage_history": parse_usage_history(find_table_after_h2(root, "Usage history")),
        "capacity_history": parse_capacity_history(find_table_after_h2(root, "Battery capacity history")),
        "life_estimates": parse_life_estimates(find_table_after_h2(root, "Battery life estimates")),
...
def aggregate_usage_monthly(history: Sequence[dict]) -> List[dict]:
    ...
    for item in history:
        ...
        while cursor < end:
            ...
            segment_seconds = max((segment_end - cursor).total_seconds(), 0)
    ...
    for key in sorted(buckets):
        result.append({"date": key, "bat": buckets[key]["bat"], "ac": buckets[key]["ac"]})
    return result
...
def chart_capacity_compare(batteries: Sequence[dict], out_dir: Path):
    valid = [b for b in batteries if b.get("design_num") and b.get("full_num")]
    if not valid:
        return None
...
def chart_capacity_history(history: Sequence[dict], out_dir: Path):
    valid = [h for h in history if h.get("start") and h.get("full") and h.get("design") and h["full"] > 0]
    if len(valid) < 2:
        return None
...
def chart_life_estimates(history: Sequence[dict], out_dir: Path):
    valid = [h for h in history if h.get("start") and h.get("fc_active") and h.get("dc_active")]
    if len(valid) < 2:
        return None
    valid = sorted(valid, key=lambda x: x["start"])
    dates = [h["start"] for h in valid]
    ...
    return _css_line_chart(
        ...
        fill_between=(0, 1), annotations=annotations, y_format="{:.1f}",
    )
...
def generate_output_html(
    ...
    for battery in batteries:
        ...
        ring = svg_ring(
            health or 0, size=134, stroke=12, color=hc,
        ...
        if battery.get("design_num"):
            details.append(("设计容量", f"{battery['design_num']:,.0f} mWh"))
        if battery.get("full_num"):
            details.append(("满充容量", f"{battery['full_num']:,.0f} mWh"))
        if battery.get("loss") is not None:
            details.append(("容量损耗", f"{battery['loss']:,.0f} mWh"))
        if battery.get("cycle_num") is not None:
            details.append(("循环次数", f"{battery['cycle_num']} 次"))
        if battery.get("loss") is not None and battery.get("cycle_num"):
            details.append(("每周期损耗", f"{battery['loss']/battery['cycle_num']:.2f} mWh/次"))
        kv_html = "".join(
            f"""
            <div class="kv">
    ...
    avg_health_values = [b["health"] for b in batteries if b.get("health") is not None]
    avg_health = (sum(avg_health_values) / len(avg_health_values) if avg_health_values else None)
    ...
    for battery in batteries:
        ...
        if h is None:
            continue
        if h < 40:
            advice.append(f"{name} 健康度仅 {h:.1f}%，已经属于严重衰减。")
        elif h < 60:
            advice.append(f"{name} 健康度为 {h:.1f}%，续航能力已经明显下降。")
        elif h < 80:
            advice.append(f"{name} 健康度为 {h:.1f}%，低于 80% 参考线，建议观察续航。")
        cycles = battery.get("cycle_num")
        if cycles is not None and cycles >= 800:
            advice.append(f"{name} 已记录 {cycles} 次循环，循环次数较高。")

    if cap_decay is not None and cap_decay > 30:
        advice.append(f"容量历史首末记录相比，满充容量累计变化约 {cap_decay:.1f}%。")
...
<div class="container">
  ...
  <div class="card">
    ...
    {chart_cards if chart_files else "<p class='explain'>暂无可视化数据。</p>"}
  </div>
...
def parse_args():
    parser = argparse.ArgumentParser(description="Windows battery-report 深度分析工具")
    parser.add_argument("input", nargs="?", help="已有 battery-report.html；不填写则自动调用 powercfg")
...
def main():
    ...
    try:
        ...
        chart_jobs = [
            ...
            ("chart_recent_activity", chart_recent_activity, (data["recent_usage"], data["battery_usage"], report_dir)),
        ]
        chart_files = []
        for name, func, args_tuple in chart_jobs:
            try:
                ...
                pass
        report_path = generate_output_html(data, chart_files, report_dir)
        if not args.input:
            try:
                Path(source).unlink()
            except OSError:
